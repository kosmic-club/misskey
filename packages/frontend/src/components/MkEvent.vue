...
<div :class="$style.container">
	...
	<dl :class="$style.details">
		...
		<template v-if="note.event!.metadata.url">
			...
			<dd :class="$style.value"><a :href="note.event!.metadata.url" target="_blank" rel="noopener">{{ note.event!.metadata.url }}</a></dd>
		</template>
		...
		<template v-if="note.event!.metadata.performer">
			...
			<dd :class="$style.value">{{ note.event!.metadata.performer.map(p => p.name).join(', ') }}</dd>
		</template>
		<template v-if="note.event!.metadata.offers?.url">
			<dt :class="$style.key">{{ i18n.ts._event.ticketsUrl }}</dt> 
			<dd :class="$style.value"><a :href="note.event!.metadata.offers.url" target="_blank" rel="noopener">{{ note.event!.metadata.offers.url }}</a></dd>
		</template>
